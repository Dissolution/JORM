...
namespace JORM.Text
{
    public ref struct StringHandler
    {
        ...
        public StringHandler(int capacity)
        {
            ...
            if (text.Length == 2)
            {
                ...
                if ((uint)pos < chars.Length - 1)
                {
                    Unsafe.WriteUnaligned(
                                          ...
                                          Unsafe.ReadUnaligned<int>(ref Unsafe.As<char, byte>(ref Unsafe.AsRef<char>(in text.GetPinnableReference()))));
                    _pos = pos + 2;
                }
                else
                {
        ...
        public void Append(string value)
        {
            ...
            if (value.Length == 2)
            {
                ...
                if ((uint)pos < chars.Length - 1)
                {
                    Unsafe.WriteUnaligned(
                        ref Unsafe.As<char, byte>(ref Unsafe.Add(ref MemoryMarshal.GetReference(chars), pos)),
                        Unsafe.ReadUnaligned<int>(ref Unsafe.As<char, byte>(ref Unsafe.AsRef<char>(in value.GetPinnableReference()))));
                    _pos = pos + 2;
                }
                else
                {
        ...
        private void AppendSpanDirect(ReadOnlySpan<char> text)
        {
            if (text.TryCopyTo(_chars.Slice(_pos)))
            {
                _pos += text.Length;
            }
            else
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            uint newCapacity = Math.Max(requiredMinCapacity, Math.Min((uint)_chars.Length * 2, 0xFFFFFF));//string.MaxLength));
